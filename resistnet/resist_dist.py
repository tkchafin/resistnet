...
def effectiveResistanceMatrix(points, inc_matrix, edge_resistance):
	r=pd.DataFrame(columns=list(points.keys()), index=list(points.keys()))
	print(points.values())
	inc_row=0
	edge_resistance=np.array(edge_resistance)
	print("r shape", edge_resistance.shape)
	for ia, ib in itertools.combinations(range(0,len(points)),2):
		...
		print(edge_resistance, flush=True)
		print(np.multiply(edge_resistance, inc))
		d=np.sum(np.multiply(edge_resistance, inc)) #effective resistance is simply a sum of serial resistances
		inc_row+=1
		r.loc[list(points.keys())[ia], list(points.keys())[ib]] = d
		r.loc[list(points.keys())[ib], list(points.keys())[ia]] = d
		sys.exit()
